...
function NoClip() {
    ...
    if (NoclipActive) {
        ...
        let zpos = game.getGroundZFor3dCoord(cpos.x,cpos.y,cpos.z,0,true,false)
        game.setEntityCoordsNoOffset(noclipEntity.scriptID, cpos.x, cpos.y, parseFloat(zpos[1]) + 2, true, true, true);
...
alt.on("keyup", (key) => {
    if (key === KeyControls.ToggleAltV) {
        if (alt.Player.local.getSyncedMeta('admin') < 1) return;
        NoClip();
    }
});
